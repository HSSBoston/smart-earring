...
while True:
    try:
        ...
        if maxVolLevel < volThreshold:
            ...
            freqSpectrum = sa.fft(soundSamples)
            freq = sa.getHighestPeakFrequency(freqSpectrum=freqSpectrum,
                                              samplingRate=44100)
            print("Volume: " + str(maxVolLevel))
            print("Frequency (Hz): " + str(freq))
            print("Note name: " + sa.freqToNoteName(freq))
